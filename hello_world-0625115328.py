...
args = {
    "project_id": "hello_world-0625115328",
}

dag = DAG(
    "hello_world-0625115328",
    default_args=args,
...
notebook_op_6a5e54ea_440a_47f0_922a_28b1550a7526 = NotebookOp(
    ...
    cos_bucket="airflow",
    cos_directory="hello_world-0625115328",
    cos_dependencies_archive="hello-6a5e54ea-440a-47f0-922a-28b1550a7526.tar.gz",
    pipeline_outputs=[],
    pipeline_inputs=[],
    image="https://quay.io/repository/ml-on-k8s/kaniko-container-builder",
    in_cluster=True,
    env_vars={
...
notebook_op_5ba230f0_0912_4fde_8907_473ee99771d1 = NotebookOp(
    ...
    pipeline_outputs=[],
    pipeline_inputs=[],
    image="https://quay.io/repository/ml-on-k8s/airflow-python-runner",
    in_cluster=True,
    env_vars={
        "AWS_ACCESS_KEY_ID": "minio",
        "AWS_SECRET_ACCESS_KEY": "minio123",
